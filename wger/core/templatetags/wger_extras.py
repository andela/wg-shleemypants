...
def get_current_settings(exercise, set_id):
    ...
    We need to do this here because it's not possible to pass
    arguments to function in
    the template, and we are only interested on the settings
...
def pagination(paginator, page):
    ...
    # For very long lists (e.g. the English ingredient with
    # more than 8000 items)
...
def is_fileupload(field):
    '''
    Tests if a field element is a file upload, as it needs
