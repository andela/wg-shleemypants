...
def dashboard(request):
    ...
    try:
        plan = NutritionPlan.objects.filter(user=request.user).latest(
            'creation_date')
...
class FeedbackClass(FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        template = 'base_empty.html' if self.request.is_ajax() else 'base.html'
        context['extend_template'] = template
        return context
    ...
    def get_form_class(self):
        ...
        if user.is_anonymous() or user.userprofile.is_temporary:
